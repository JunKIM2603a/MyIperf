...
int main(int argc, char* argv[]) {
    ...
    for (int i = 1; i <= repetitions; ++i) {
        ...
        // Calculate cumulative statistics from previous iterations
        size_t total_tests_so_far = total_run_results.size();
        size_t total_passes_so_far = 0;
        ...
        size_t total_fails_so_far = total_tests_so_far - total_passes_so_far;
        
        std::cout << "--- Starting Iteration " << i << " of " << repetitions << " ---" << std::endl;
        if (i > 1) {
            std::cout << "--- Cumulative: " << total_tests_so_far << " tests completed ("
                      << total_passes_so_far << " passed, " << total_fails_so_far << " failed) ---" << std::endl;
        }
        std::cout << "=================================================" << std::endl;
