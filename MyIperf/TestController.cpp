...
const std::chrono::seconds TestController::HANDSHAKE_TIMEOUT(2);
const std::chrono::seconds TestController::INITIAL_TIMEOUT(15);
...
const char* stateToString(TestController::State state) {
    switch (state) {
        ...
        case TestController::State::SENDING_FINAL_ACK: return "SENDING_FINAL_ACK";
        ...
        case TestController::State::WAITING_FOR_FINAL_ACK: return "WAITING_FOR_FINAL_ACK";
...
/**
 * @brief Periodically checks for timeouts during the handshake process.
 */
void TestController::update() {
    std::lock_guard<std::mutex> lock(m_stateMachineMutex);

    if (currentState == State::IDLE || currentState == State::FINISHED || currentState == State::ERRORED) {
        return; // No timeouts needed for terminal or idle states.
    }

    auto now = std::chrono::steady_clock::now();
    
    switch (currentState) {
        case State::ACCEPTING: // Server is waiting for a client to connect
            if (std::chrono::duration_cast<std::chrono::seconds>(now - m_initialStateTime) > INITIAL_TIMEOUT) {
                Logger::log("Error: Server timed out waiting for a client connection.");
                transitionTo_nolock(State::ERRORED);
            }
            break;

        case State::WAITING_FOR_CONFIG: // Server is waiting for the client's config packet
            if (std::chrono::duration_cast<std::chrono::seconds>(now - m_initialStateTime) > INITIAL_TIMEOUT) {
                Logger::log("Error: Server timed out waiting for client configuration.");
                transitionTo_nolock(State::ERRORED);
            }
            break;

        case State::WAITING_FOR_ACK: { // Client is waiting for server's CONFIG_ACK
            auto elapsed = std::chrono::duration_cast<std::chrono::seconds>(now - lastPacketSentTime);
            if (elapsed > HANDSHAKE_TIMEOUT) {
                if (retryCount >= MAX_RETRIES) {
                    Logger::log("Error: Handshake failed. No response from server after " + std::to_string(MAX_RETRIES) + " retries.");
                    transitionTo_nolock(State::ERRORED);
                } else {
                    retryCount++;
                    Logger::log("Warning: Handshake timeout. Retrying... (Attempt " + std::to_string(retryCount) + ")");
                    // Resend the config packet by re-entering the SENDING_CONFIG state.
                    transitionTo_nolock(State::SENDING_CONFIG);
                }
            }
            break;
        }
        case State::WAITING_FOR_FINAL_ACK: { // Server is waiting for client's FINAL_ACK
            auto elapsed = std::chrono::duration_cast<std::chrono::seconds>(now - lastPacketSentTime);
            if (elapsed > HANDSHAKE_TIMEOUT) {
                Logger::log("Error: Handshake failed. Did not receive final acknowledgment from client.");
                transitionTo_nolock(State::ERRORED);
            }
            break;
        }
        default:
            // No timeout logic needed for other states.
            break;
    }
}
...
void TestController::transitionTo_nolock(State newState) {
    ...
    switch (newState) {
        ...
        case State::ACCEPTING: {
            ...
            DebugPause(string_format("[%s:%d] State::%s",  __FUNCTION__, __LINE__,stateToString(newState)));
            m_initialStateTime = std::chrono::steady_clock::now();
        ...
        case State::SENDING_CONFIG: {
            ...
            // Reset retry counter and set timestamp for timeout tracking.
            retryCount = 0;
        ...
        case State::SENDING_FINAL_ACK: {
            Logger::log("Info: Client sending final acknowledgment.");
            DebugPause(string_format("[%s:%d] State::%s", __FUNCTION__, __LINE__, stateToString(newState)));

            PacketHeader header{};
            header.startCode = PROTOCOL_START_CODE;
            header.messageType = MessageType::FINAL_ACK;
            std::vector<char> packet(sizeof(PacketHeader));
            memcpy(packet.data(), &header, sizeof(PacketHeader));

            networkInterface->asyncSend(packet, [this](size_t bytesSent) {
                if (bytesSent > 0) {
                    Logger::log("Info: Client sent FINAL_ACK successfully.");
                    // The handshake is now complete from the client's perspective.
                    transitionTo_nolock(State::RUNNING_TEST);
                } else {
                    Logger::log("Error: Client failed to send FINAL_ACK.");
                    transitionTo_nolock(State::ERRORED);
                }
            });
            break;
        }
        case State::WAITING_FOR_FINAL_ACK: {
            Logger::log("Info: Server waiting for final client acknowledgment.");
            DebugPause(string_format("[%s:%d] State::%s", __FUNCTION__, __LINE__, stateToString(newState)));
            // The timer is started when the CONFIG_ACK is sent in onPacket.
            break;
        }
        case State::RUNNING_TEST: {
            ...
            if (currentConfig.getMode() == Config::TestMode::CLIENT) {
                ...
                // Stats are now reset in onPacket when the handshake completes.
            }
            break;
        }
    ...
    if (currentConfig.getMode() == Config::TestMode::SERVER) {
        switch (header.messageType) {
            case MessageType::CONFIG_HANDSHAKE:
                ...
                if (currentState == State::WAITING_FOR_CONFIG) {
                    ...
                    try {
                        ...
                        networkInterface->asyncSend(ackPacket, [this](size_t bytesSent) {
                            if (bytesSent > 0) {
                                ...
                                // Start the timeout timer for waiting for the final ACK
                                lastPacketSentTime = std::chrono::steady_clock::now();
                                transitionTo_nolock(State::WAITING_FOR_FINAL_ACK);
                            } else {
                ...
                } else if (currentState == State::WAITING_FOR_FINAL_ACK) {
                    Logger::log("Warning: Received duplicate CONFIG_HANDSHAKE from client. Resending ACK.");
                    ...
                    networkInterface->asyncSend(ackPacket, [this](size_t bytesSent) {
                        if (bytesSent > 0) {
                            Logger::log("Info: Server re-sent config ACK.");
                            // Reset the timeout timer
                            lastPacketSentTime = std::chrono::steady_clock::now();
                        } else {
                            Logger::log("Error: Server failed to re-send config ACK.");
                            transitionTo_nolock(State::ERRORED);
                        }
                    });
                }
                break;
            case MessageType::FINAL_ACK:
                DebugPause(string_format("[%s:%d] MessageType::%s", __FUNCTION__, __LINE__, MessageTypeToString(header.messageType)));
                if (currentState == State::WAITING_FOR_FINAL_ACK) {
                    Logger::log("Info: Server received FINAL_ACK. Handshake complete. Starting test measurement.");
                    packetReceiver->resetStats();
                    m_testStartTime = std::chrono::steady_clock::now();
                    transitionTo_nolock(State::RUNNING_TEST);
                }
            ...
            case MessageType::DATA_PACKET:
                if (currentState == State::WAITING_FOR_FINAL_ACK) {
                    Logger::log("Warning: Server received DATA_PACKET before FINAL_ACK. Assuming handshake complete. Starting test measurement.");
                    packetReceiver->resetStats();
                    m_testStartTime = std::chrono::steady_clock::now();
                    // Manually account for the packet that triggered the state change, as resetStats wiped it.
                    packetReceiver->accountForPacket(header, payload);
                    transitionTo_nolock(State::RUNNING_TEST);
                }
                // Data packets in other states are handled by the receiver's stats automatically.
                break;
        ...
        switch (header.messageType) {
            case MessageType::CONFIG_ACK:
                ...
                if (currentState == State::WAITING_FOR_ACK) {
                    Logger::log("Info: Client received server ACK. Sending final ACK.");
                    retryCount = 0; // Reset retry counter as handshake is progressing.
                    transitionTo_nolock(State::SENDING_FINAL_ACK);
                }
                break;
            case MessageType::TEST_FIN:
                DebugPause(string_format("[%s:%d] MessageType::%s", __FUNCTION__, __LINE__, MessageTypeToString(header.messageType)));
