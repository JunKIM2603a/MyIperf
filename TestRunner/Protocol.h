...
enum class MessageType {
  ...
  SERVER_SHUTDOWN   // Client -> Server: Shut down the server
};
...
inline MessageType StringToMessageType(const std::string &str) {
  if (str == "CONFIG_REQUEST")
    return MessageType::CONFIG_REQUEST;
  if (str == "SERVER_READY")
    return MessageType::SERVER_READY;
  if (str == "TEST_COMPLETE")
    return MessageType::TEST_COMPLETE;
  if (str == "RESULTS_REQUEST")
    return MessageType::RESULTS_REQUEST;
  if (str == "RESULTS_RESPONSE")
    return MessageType::RESULTS_RESPONSE;
  if (str == "ERROR_MESSAGE")
    return MessageType::ERROR_MESSAGE;
  if (str == "HEARTBEAT")
    return MessageType::HEARTBEAT;
  if (str == "SERVER_SHUTDOWN")
...
// Protocol constants
namespace Protocol {
constexpr int DEFAULT_CONTROL_PORT = 9500;
constexpr int DEFAULT_TEST_PORT = 5201;
...
inline std::string SessionStateToString(SessionState state) {
  ...
  case SessionState::IDLE:
    return "IDLE";
  case SessionState::CONFIG_RECEIVED:
    return "CONFIG_RECEIVED";
  case SessionState::SERVER_STARTING:
    return "SERVER_STARTING";
  case SessionState::SERVER_READY:
    return "SERVER_READY";
  case SessionState::TESTING:
    return "TESTING";
  case SessionState::TEST_COMPLETE:
    return "TEST_COMPLETE";
  case SessionState::RESULTS_SENT:
    return "RESULTS_SENT";
