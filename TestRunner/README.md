# TestRunner - 분산 IPEFTC 테스트 시스템

TestRunner는 TestRunner에 TCP 기반 원격 제어 기능을 추가한 분산 테스트 시스템입니다. 서로 다른 머신에서 IPEFTC 서버와 클라이언트를 원격으로 제어하고 동기화하여 네트워크 성능 테스트를 수행할 수 있습니다.
...
- **단일/멀티 포트 지원**: 하나 또는 여러 포트를 순서대로 테스트 실행
...
├─────────────────────────────────────────────────────────────┤
│                                                               │
│  IPEFTC Client            TCP 5201+            IPEFTC Server  │
│  (로컬 머신)          <═══════════════════>      (원격 머신)   │
│                      성능 테스트 수행                          │
│                                                               │
└─────────────────────────────────────────────────────────────┘
...
TestRunner는 단일 실행 파일로 서버/클라이언트 모드를 선택하여 사용합니다.

```bash
TestRunner.exe --mode <server|client> [옵션]
```
...
**기본 실행:**
```bash
.\TestRunner.exe --mode server
```

**포트 지정:**
```bash
.\TestRunner.exe --mode server --control-port 9000
```

**IPEFTC 경로 지정:**
```bash
.\TestRunner.exe --mode server --control-port 9000 --ipeftc-path "C:\path\to\IPEFTC.exe"
```
...
**기본 실행:**
```bash
.\TestRunner.exe --mode client --server 192.168.1.100
```
...
여러 포트를 순서대로 테스트합니다. 서버 ControlServer는 control client별 worker thread를
지원하지만, Windows IOCP 기반 IPEFTC child process 안정성을 위해 TestRunner client는
포트별 IPEFTC 실행을 직렬화합니다.
...
포트는 `--test-port`부터 순차적으로 할당됩니다 (예: 5201, 5202, 5203, 5204, 5205).
...
| `--ipeftc-path` | IPEFTC.exe 경로 | 자동 탐색 (`IPEFTC.exe`, `MyIperf\\build\\bin\\Release\\IPEFTC.exe`, `MyIperf\\build\\bin\\Debug\\IPEFTC.exe`) |

`--ipeftc-path`를 생략하면 TestRunner는 현재 작업 디렉터리와 알려진 MyIperf 빌드 출력 위치를 순서대로 탐색합니다.
탐색에 실패하면 후보 경로 목록을 출력하므로, 그때 명시 경로를 지정하면 됩니다.
...
|------|------|--------|
| `--server` | 서버 IP 주소 | **필수** |
| `--server-bind` | 원격 IPEFTC 서버가 bind할 IP 주소 | `0.0.0.0` |
| `--test-port` | IPEFTC 테스트 포트 | 5201 |
| `--packet-size` | 패킷 크기 (바이트) | 8192 |
| `--num-packets` | 전송할 패킷 개수 | 10000 |
| `--interval-ms` | 패킷 전송 간격 (ms) | 0 (최대 속도) |
| `--save-logs` | 로그 저장 여부 (true/false) | true |
| `--num-ports` | 테스트할 포트 수 (멀티 포트 모드) | 1 (단일) |
| `--total-runs` | 전체 테스트 실행 횟수 (각 실행 후 프로그램 재시작) | 1 |

> Windows dynamic port range(일반적으로 49152-65535)와 겹치는 포트를 `--test-port`로 쓰면, 로컬 client/control 연결의 임시 포트와 충돌해 server bind가 실패할 수 있습니다. 기본값은 MyIperf 기본 포트와 같은 `5201`입니다.

## 프로토콜 명세

### 메시지 타입

TestRunner는 JSON 기반 메시지 프로토콜을 사용합니다.
...
**로컬 클라이언트:**
```bash
.\TestRunner.exe --mode client --server 192.168.1.100 --test-port 5201 --num-packets 10000
```
...
[ControlServer] Test config received - Port: 5201, Packets: 10000, Size: 8192
...
[ControlClient] Sending CONFIG_REQUEST for port 5201
[ControlClient] Server ready on port 5201
...
--------------------------------------------------------------------------------------------------------
Server  5201    2.35           278.63              81920000              10000                 PASS
Client  5201    2.36           277.42              81920000              10000                 PASS
...
==================================================
[TestRunner] Started new process for run 2
[TestRunner] Exiting current process. Resources will be released.
```
...
2. 서버는 다중 control client를 worker thread로 처리하지만, TestRunner client는 안정성을 위해 포트별 IPEFTC 실행을 직렬화합니다
...
## TestRunner vs TestRunner

| 기능 | TestRunner | TestRunner |
|------|-----------|-------------|
