...
void from_json(const json &j, TestConfig &c) {
  ...
  if (j.contains("serverBindIP")) {
    j.at("serverBindIP").get_to(c.serverBindIP);
  }
...
void AnalyzeTestResult(TestResult &result, long long expectedPackets,
  ...
  if (expectedPackets > 0 && result.totalPackets != expectedPackets) {
    passed = false;
    reason +=
  ...
  if (expectedBytes > 0 && result.totalBytes != expectedBytes) {
    passed = false;
    reason += "Byte count mismatch (Rx: " + std::to_string(result.totalBytes) +
              ", Exp: " + std::to_string(expectedBytes) + "); ";
  }

  if (result.totalPackets <= 0) {
    passed = false;
    reason += "No packets parsed from IPEFTC output; ";
  }
