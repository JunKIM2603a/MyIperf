...
void ControlServer::Start() {
  ...
  while (running.load()) {
    ...
    clientThreads.emplace_back(&ControlServer::HandleClient, this,
                               clientSocket);
...
void ControlServer::JoinClientThreads() {
...
void ControlServer::PrintFinalSummary() {
  std::vector<SessionRecord> history;
  {
    std::lock_guard<std::mutex> lock(sessionHistoryMutex);
  ...
  std::cout << "\n========================================================"
            << std::endl;
  std::cout << "          FINAL GLOBAL SUMMARY (Server-side)" << std::endl;
  std::cout << "========================================================" << std::endl;
    ...
    while (true) {
      ...
      if (type == MessageType::CONFIG_REQUEST) {
        ...
        if (ProcessManager::GetInstance().LaunchIPEFTCServer(
          ...
          if (!ProcessManager::GetInstance().WaitForServerReady(
                  ipeftcProcess, Protocol::SERVER_START_TIMEOUT_MS)) {
            ErrorMessage err("IPEFTC server failed to become ready");
          ...
          resp.serverIP = req.config.serverBindIP;
...
bool ControlServer::SendMessage(SOCKET socket,
  ...
  if (!SendAll(socket, (const char *)&networkLen, 4))
    return false;

  // 2. Send Data
  if (!SendAll(socket, serializedMsg.c_str(), (int)serializedMsg.size()))
    return false;
...
std::string ControlServer::ReceiveMessage(SOCKET socket) {
  // 1. Receive Length
  uint32_t networkLen;
  if (!RecvAll(socket, (char *)&networkLen, 4))
  ...
  if (!RecvAll(socket, buffer.data(), (int)len))
    return "";
